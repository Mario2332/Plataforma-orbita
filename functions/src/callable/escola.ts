...
/**
 * Helper: Obter tenantId da escola autenticada
 */
async function getEscolaTenantId(uid: string): Promise<string> {
  const escolaDoc = await db.collection("escolas").doc(uid).get();
  if (!escolaDoc.exists) {
  ...
  if (!tenantId) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      "Escola não possui tenantId configurado"
    );
  }
...
/**
 * Helper: Validar que um aluno pertence ao tenant e escola
 */
async function validateAlunoAccess(
  ...
  if (alunoData.tenantId !== tenantId || alunoData.escolaId !== escolaId) {
    throw new functions.https.HttpsError(
      "permission-denied",
...
const getMe = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    return { id: escolaDoc.id, ...escolaDoc.data() };
  });

/**
 * Listar alunos da escola
 * ISOLAMENTO POR TENANT: Retorna apenas alunos do mesmo tenant e da mesma escola
 */
const getAlunos = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // Filtrar por tenantId E escolaId para garantir isolamento
...
const getAlunoById = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    // ISOLAMENTO: Validar que o aluno pertence ao mesmo tenant e à mesma escola
    if (alunoData.tenantId !== tenantId || alunoData.escolaId !== auth.uid) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Você não tem permissão para acessar este aluno"
      );
...
const createAluno = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      // Criar documento do usuário com tenantId
      await db.collection("users").doc(userRecord.uid).set({
        uid: userRecord.uid,
        email,
        name: nome,
        role: "aluno",
        tenantId: tenantId, // ISOLAMENTO: Adicionar tenantId
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        lastSignedIn: admin.firestore.FieldValue.serverTimestamp(),
      });

      // Criar documento do aluno com tenantId
...
/**
 * Obter configurações da plataforma do escola
 */
const getConfig = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!escolaDoc.exists) {
      throw new functions.https.HttpsError("not-found", "Escola não encontrado");
    }

    const escolaData = escolaDoc.data()!;
    return {
      nomePlataforma: escolaData.nomePlataforma || "",
      logoUrl: escolaData.logoUrl || "",
      corPrincipal: escolaData.corPrincipal || "#3b82f6",
    };
  });

/**
 * Atualizar configurações da plataforma do escola
 */
const updateConfig = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      await db.collection("escolas").doc(auth.uid).update(updates);

      return { success: true };
    } catch (error: any) {
      functions.logger.error("Erro ao atualizar configurações do escola:", error);
      throw new functions.https.HttpsError("internal", error.message);
    }
  });
...
/**
 * Obter dados completos do aluno para visualização pelo escola
 */
...
/**
 * Funções para o escola gerenciar estudos do aluno
 */
...
/**
 * Funções para o escola gerenciar simulados do aluno
 */
...
/**
 * Funções para o escola gerenciar horários do cronograma do aluno
 */
...
/**
 * Funções para o escola gerenciar templates de cronograma do aluno
 */
...
/**
 * Funções para o escola gerenciar diário emocional do aluno
 */
...
/**
 * Funções para o escola gerenciar autodiagnósticos do aluno
 */
...
/**
 * Funções para o escola gerenciar progresso nos conteúdos ENEM do aluno
 */
...
/**
 * Funções para o escola gerenciar cronogramas do aluno
 */
...
/**
 * Funções para o escola gerenciar tarefas do cronograma do aluno
 */
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      const metaData: any = {
        ...
        createdBy: auth.uid, // ID do escola que criou
      };
...
/**
 * Atualizar meta do aluno (escola)
 */
...
// Exportar todas as funções do escola
export const escolaFunctions = {
  // Funções básicas do escola
  getMe,
  getAlunos,
  getAlunoById,
  createAluno,
